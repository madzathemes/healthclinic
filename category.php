...
<div class="container mt-content-container">
	<div class="row">
		<div class="col-md-12">
			...
			<?php if($grid!=0) { echo do_shortcode('[grid type="'.esc_attr($grid).'" title="'. esc_html__( 'Category','healthclinic' ) .': '.esc_attr($cat_title).'" position="left" title_type="left" category="'.esc_attr($cat_slug).'"  ]'); ?>
	...
	<div class="col-md-8  floatleft">
		<?php if ( have_posts() ) {
			 	...
			 	if($grid==0) {
					echo do_shortcode('[posts pagination=on item_nr='.esc_attr($default_posts_per_page).' offset='.esc_attr($offset).'  category="'.esc_attr($cat_slug).'" type='.esc_attr($post_style).' title="'. esc_html__( 'Category','healthclinic' ) .': '.esc_attr($cat_title).'" title_type=left ]');
				} else {
