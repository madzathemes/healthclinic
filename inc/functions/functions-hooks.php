<?php
function healthclinic_css() {

	wp_enqueue_style('healthclinic-style', get_stylesheet_uri());
	...
	if(!empty($options['colors_default'])){
	 ...
	 if ( $custom_styles != '' ) {
	  $css = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $custom_styles);
		wp_add_inline_style( 'healthclinic-style', $css );
	}

}
add_action( 'wp_enqueue_scripts', 'healthclinic_css');

function healthclinic_header_script() {


		$option = get_option("healthclinic_theme_options");

		wp_enqueue_script( 'healthclinic_script', get_template_directory_uri(). '/inc/js/scripts.js', array( 'jquery'), '', true );
		wp_localize_script( 'healthclinic_script', 'ajax_posts', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'noposts' => esc_html__('No older posts found', 'healthclinic'), ));
    ...
    function healthclinic_fonts_url() {
      ...
      $theme_font = "Nunito:200,400,600,700,900";
         ...
         */
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'healthclinic' ) ) {
            $font_url = add_query_arg( 'family', urlencode( ''. esc_attr($theme_font) .'' ), "//fonts.googleapis.com/css" );
        }
        return $font_url;
    }
    wp_enqueue_style( 'healthclinic-fonts', healthclinic_fonts_url(), array(), '1.0.0' );
}
add_action('wp_enqueue_scripts', 'healthclinic_header_script');




function healthclinic_admin_script() {
	wp_enqueue_style('healthclinic-admin', get_template_directory_uri().'/inc/css/admin.css');
}
add_action('admin_enqueue_scripts', 'healthclinic_admin_script');


add_filter('body_class','healthclinic_class');
function healthclinic_class($classes) {
