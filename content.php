...
<article id="post-<?php the_ID(); ?>" class="mt-post-article <?php if ( is_sticky() and !is_single()){ ?> post_sticky <?php } ?>">
	...
	<?php if (!is_single()){ ?>
		<header class="entry-header">
			<h2 class="entry-title">
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', 'healthclinic' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php echo get_the_title();  ?></a>
			</h2>
		</header>
	<?php } ?>
	...
	<div class="entry-content">
		...
		?>
		<a class="mt-readmore-url mt-btn mt-btn-1 mt-radius" href="<?php echo get_permalink(); ?>"><?php echo esc_html__( 'Read More', 'healthclinic' ); ?></a>
